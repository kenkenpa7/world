...
// GROQ Query to fetch countries
const getCountries = async () => {
  const query = `*[_type == "country"] | order(name asc) {
    name,
    "slug": slug.current,
    currency,
    cashRatio,
...
export const revalidate = 60; // 60秒ごとにキャッシュ更新（ISR）

export default async function Home() {
  const countries = await getCountries();

  // もしデータが0件の場合はダミーを表示するフォールバック
  const displayCountries = countries.length > 0 ? countries : [
    { name: "まだ国が登録されていません", slug: "#", currency: "-", cashRatio: 0, bestExchange: "-", atmSafety: "-" }
  ];
      ...
      <div className="container mx-auto max-w-5xl px-6 flex flex-col items-center relative z-10 pointer-events-none pt-32 pb-24">
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-6 pointer-events-auto">
          {displayCountries.map((country: any, i: number) => (
            <AnimateIn key={country.slug} delay={0.5 + (i * 0.1)}>
